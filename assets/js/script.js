  ...
  -------------------------------------------*/
   ...
   -------------------------------------------*/
    ...
    document.addEventListener("DOMContentLoaded", function () {
        const leftShapeContainer = document.querySelector(".left-shape");
        const rightShapeContainer = document.querySelector(".right-shape");

        const shapeImages = [
            "assets/images/slider/ishape-1.svg",
            "assets/images/slider/ishape-2.svg",
            "assets/images/slider/ishape-3.svg",
            "assets/images/slider/ishape-4.svg",
            "assets/images/slider/ishape-5.svg",
        ];

        function createFallingShape(container) {
            ...
            // Vị trí ngẫu nhiên
            if (container.classList.contains("left-shape")) {
                shapeDiv.style.left = `${Math.random() * 20}%`;
            } else if (container.classList.contains("right-shape")) {
                shapeDiv.style.right = `${Math.random() * 20}%`;
            }

            // Kích thước ngẫu nhiên
            const size = Math.random() * 100 + 50; // 20px - 60px
            shapeDiv.style.width = `${size}px`;
            shapeDiv.style.height = `${size}px`;

            // Animation ngẫu nhiên
            const duration = Math.random() * 50 + 50; // 5s - 10s
            const delay = Math.random() * 1; // 0s - 5s
            shapeDiv.style.animationDuration = `${duration}s`;
            shapeDiv.style.animationDelay = `${delay}s`;

            container.appendChild(shapeDiv);

            // Xóa shape khi animation kết thúc
            setTimeout(() => {
                container.removeChild(shapeDiv);
            }, (duration + delay) * 1000);
        }

        function spawnShapes() {
            setInterval(() => {
                createFallingShape(leftShapeContainer);
                createFallingShape(rightShapeContainer);
            }, 2000); // Mỗi giây tạo 1 shape mới
        }
    ...
    -------------------------------------------*/
    if ($("#clock2").length) {
        $("#clock2").countdown("2025-03-15 00:00:00", function (event) {
            var $this = $(this).html(
                event.strftime(
                    "" +
